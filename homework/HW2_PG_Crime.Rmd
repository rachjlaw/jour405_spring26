...
The most common incidents have something to do with automotives (accident, accident with impound, stolen auto, theft from auto). I'd consider these categories more important than the others because they are strikingly more common than the others, and are somewhat related. I think some of the categories that separate different types of the same crime are also somewhat difficult to understand, mostly because there are some examples that could fit in both categories, and some that don't make sense. For example, if one isn't robbing from a commerical place, a residence, or a vehicle, what are they robbing? Are automotives stolen and recovered counted in "auto stolen?" Is stealing someone's eletric scooter theft or auto theft?
...
```{r}
total_incidents <- pg_crime |> mutate( = (`Fall 2024`- `Fall 2021`)/`Fall 2021` * 100)
```
