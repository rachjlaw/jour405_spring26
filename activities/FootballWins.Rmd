...
I'd say this model is useful because it sort of acts as a check for the coefficients. The residuals shows concisely that even though there are deviations, the model is accurate and helpful. 
...
```{r}
model_2 <- lm(Wins ~ `Yards Allowed` + Takeaways + `Points Scored`, data = teams)
summary(model_2)

```
```{r}
selected_vars <- teams |>  select(`Yards Allowed`, Takeaways, `Points Scored`, Wins)
cor(selected_vars)
corr_matrix <- cor(selected_vars)
corrplot(corr_matrix, method = "circle")
```
...
I specifically designed this model with defense in mind, and it shows that there isn't super significant correlation between yards allowed and points scored, or takeaways and wins. To me, this shows that defense does not in fact win championships, as there isn't a lot of correlation between actually wining and intense parts of defense. 
